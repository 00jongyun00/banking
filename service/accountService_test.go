...
import (
	realdomain "bangking/domain"
	"bangking/dto"
	"bangking/mocks/domain"
	"testing"
...
func Test_shoud_return_an_error_from_the_server_side_if_the_new_account_cannot_be_created(t *testing.T) {
	// Arrange
	// ctrl := gomock.NewController(t)
	// defer ctrl.Finish()
	// mockRepo := domain.NewMockAccountRepository(ctrl)
...
func Test_should_return_new_account_response_whe_a_new_account_is_saved_successfully(t *testing.T) {
	// Arrange
	teardown := setup(t)
	defer teardown()
